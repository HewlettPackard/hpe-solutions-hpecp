...
"""
This script performs the Physical Node Labelling
"""
import json
import subprocess
import os
...
def get_node_label(node_fqdn_or_ip):
    ...
    try:
        ...
        command = "kubectl get node " + node_fqdn_or_ip + " --show-labels"
...
def label_for_health(vault_key):
    ...
    try:
        ...
        for key, value in data.items():
            command = "kubectl label node " + key + " health=" + value + \
                " --overwrite=true"
            res = Popen(command, stdin=PIPE, stdout=PIPE, stderr=PIPE, \
                env=user_env, shell=True)
...
def label_for_security(vault_key):
    ...
    try:
        ...
        for key, value in data.items():
            command = "kubectl label node " + key + " security=" + value + \
                " --overwrite=true"
            res = Popen(command, stdin=PIPE, stdout=PIPE, stderr=PIPE, \
                env=user_env, shell=True)
            result = res.communicate('y\ny\ny\n'.encode('utf8'))
